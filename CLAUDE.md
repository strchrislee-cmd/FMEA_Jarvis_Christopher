...
- **Step 5 리스크 화면 개선**: S/O/D 한국어 병기(중앙 라벨 맵 `help.ts` `SOD_LABELS`/`RPN_HINT` 재사용) · RPN 색상 구간(≤100 녹/101~200 주황/≥201 적, **값+라벨 병행**=색약 대응, `risk.ts` `rpnBand`) · **S=9·10 안전행은 RPN 무관 ⚠강조**(`isSafetyRow`) · 등급 선택 시 척도표 문구 2초 토스트(scales에서만 읽고 없으면 "기준 미정의", **위치=세로화면 우측 중앙**) · AP 빈 표 안내 + 등록 조합 수 표시.
